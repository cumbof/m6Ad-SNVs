...
with open("RMVar_Human_Clinvar_info.txt") as m:
...
with open("RMVar_Human_basic_info_m6A.txt") as m:
    header = m.readline().strip().split("\t")
    for line in m:
        line = line.strip()
        if line:
...
with open("clinvar_m6A.vcf", "w+") as clinvar_m6A:
    with open("clinvar.vcf") as clinvar:
        for line in clinvar:
            if line.strip():
                if line.startswith("#"):
